...
Маппер должен находится между слоем web и service, т.е. он переводит из внутреннего представления `@Entity`, с которым
работает service во внешнюю модель, с которой работает слой web. И он это должен делать по уже сформированному объекту.

Т.е. если есть два метода API, отличающихся наполнением ответа (в одном есть вложеннный объект address, в другом нет),
то это будут два маппера, которые будут понимать нужно ли обращаться к полю address в `@Entity`. Т.к. MapStruct требует
только описание интерфейсов, это просто будет два метода, отличающихся аннотацией
`@Mapping(target = "address", ignore = true)`, всю остальную работу сделает MapStruct.

##### Маппинг DTO -> `@Entity`

Обратное преобразование сложнее. Предположим, у нас есть сложный объект, который мы хотим модифицировать.

На вход маппера мы получаем DTO, по id поднимаем `@Entity`, обновляем ее, а по всем вложенным объектам мы тоже должны
поднять сущности и обновить. А после этого выполнить сохранение главного объекта и всех вложенных. Для этого потребуется
использовать `cascade`, а значит мы завязываемся на внутреннюю магию Hibernate.
...
Оба примера требуют большого количества кода в `@Mapper` (объект под управлением MapStruct) и работы внутрии сессии
Hibernate, т.к. требуется поднимать данные из БД в коде маппера, т.е. часть кода будет написана в srv/java, а другая
часть будет сгенерирована, что сделает результат непрозрачным.
