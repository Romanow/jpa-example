...
И вообще основное правило всего - подерживать структуру сервисов, мапперов, репозиториев в соответсвие с доменной
моделью.
...
в реализации этой аннотации не будет, то по факту транзакция создасться (т.к. Spring увидит `@Transactional` в
...
архитектуры нашего приложения, а значит он содержит бизнес логику, а следовательного его нужно тестировать.

Мапперы следует делать быть максмально простыми, в идеальном случае они должны заниматься только перекладыванием плоских
полей (`String`, `Integer`, `BigDecimal`) из объекта в объект. Если объект является составным, то для каждой доменной
сущности должен быть написан свой маппер. Структрурная зависмость маперов должна повторять структру доменной области для
простоты понимания, поддерджки и соблюдения Single Responsibility Principle.

Весь мапинг строить в виде "звезды" _от доменной сущности_, то есть, избегать мапинг DTO1 -> DTO2, – это упрощает
поддержку быстроменяющихся DTO. Также могут получиться такие зависисти DTO1 -> DTO2 -> `@Entity`, тогда придется
...
* Для создания можно полностью использовать маппер, а на уровне `@Entity` на поля `@OneToMany`, `@ManyToOne`,
  `@ManyToMany` проставить `cascade`, чтобы Hibernate по цепочке создал вложенные объекты и привязал их к основному.
  Т.к. здесь используется Hibernate, эта операция должны выполняться в транзакции.
* Операцию обновления сложной сущности нужно делать руками, используя MapStruct только для перекладывания плоских полей.
    * Если требуется обновить сущность `@ManyToOne`, то просто переходим в связную сущность и обновляем в ней
      необходисые поля, с помощью `cascade = CascadeType.MERGE` Hibernate выполнит обвноление связной сущности.
    * Если выполняется частичное обновление (метод PATCH) и требуется обновить массив записей `@OneToMany`, значит надо
      ...
      помощью `cascade = CascadeType.MERGE` Hibernate выполнит обвноление связной сущности.
    * Если выполняется полное обновление (метод PUT), то среди существующих записей ищутся все записи по ID из запроса:
        * найденные записи обвновляются;
        * с отсутствующих записях убирается связь с главной сущностью и с помощью `orphanRemoval = true` Hibernate их
          удаляет;
...
Если в рамках бизнес операции, реализуемой в методе на уровне серивиса, присутствует вызов ко внейшней системе (HTTP
